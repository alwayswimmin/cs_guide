...
This is implemented in \texttt{Arrays.sort()} and \texttt{Collections.sort()}.
...
\item \texttt{LinkedList} -- no explicit equivalent, but see \texttt{queue}
...
The slow part of the $O(V^2)$ formulation is the linear search for the vertex $v$ with the minimum $dist(v)$. We happen to have a data structure that resolves this problem -- a binary heap. However, we can't simply use the standard library heap, as we need to be able to change the value of the distances once they are already in the heap, or \textit{decrease-key}. This is a pretty simple function to add, however, if you have a heap already coded. Once we have this implemented, we achieve $O(E \log{V})$, as we do $E+V$ updates to our heap, each costing $O(V)$.
...
It turns out for our specific sum problem that we can do as good as $O(\log{n})$ with a \textit{segment tree}, or \textit{range tree}, or \textit{static BBST}. The essential idea is still the same -- we want to group elements in some way that allows us to update and query efficiently.
...
I cheated with my example by using a nice power of two, $n=16$, as the number of elements in the sequence. Of course, the size is not always this nice. One solution is to simply pad the back of the sequence with enough 0s and pretend that the number of elements in the sequence is actually a power of two. For the segment tree, this is not necessary -- if the root vertex is associated with the range $[1,n]$, we can simply split this range into two, $\left[1, \floor{\frac{n}{2}}\right]$ and $\left[\floor{\frac{n}{2}}+1,n\right]$. We then recursively apply this pattern except for when the lower bound of the range is equal to the upper bound. If $n=12$, the resulting tree would have the following structure.
...
I prefer this approach over padding the sequence with zeros. We see that the segment tree structure does not have to resemble a complete tree at all. However, with this approach, it is still quite balanced, so we can store a segment tree within an array as we would a heap.
...
The real magic in the Fenwick tree is how easy it is to code. The only tricky part is finding exactly what $2^{v_2(i)}$ is. But it turns out, by the way bits are arranged in negative numbers, this is just \texttt{i \& -i}. With this in mind, here's all the code that's necessary to code a Fenwick tree.
