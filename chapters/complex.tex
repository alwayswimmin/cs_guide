...
$-2$ represents the sum $x_7+x_8$, $-10$ the sum $x_9+x_{10}+x_{11}+x_{12}$, $14$ the sum $x_{13}+x_{14}$, and $0$ represents the single element $x_15$. It seems we always want to take the \textit{largest} segments that stay within the range $[7,15]$. But how do we know exactly which segments these are?

We handle queries using a recursive call, starting at the root of the tree. We always begin by querying the node for $[1,n]$. The key idea is if the the interval associated with a node is completely contained within the interval we are querying, we simply return the sum associated with that node. Otherwise, we pass the query on to each of the two children.

\noindent \begin{minipage}{\textwidth}
\begin{algorithmic}
\Function{Query}{range $[i,j]$, range $[a,b]$}
	\Comment want $\sum_{k=i}^j x_i$, querying node $[a,b]$
	\If{$j < a$ or $b < i$}
		\Comment $[a, b] \cap [i,j] = \emptyset$
		\State \Return 0
	\EndIf
	\If{$i \le a$ and $b \le j$}
		\Comment $[a,b] \subseteq [i,j]$
		\State \Return $sum(a,b)$
	\EndIf
	\State \Return $\Call{Query}{[i,j], \left[a,\floor{\frac{a+b-1}{2}}\right]} + \Call{Query}{[i,j], \left[\floor{\frac{a+b-1}{2}}+1,b \right]}$
\EndFunction
\end{algorithmic}
\end{minipage}
...
If we wanted to change the third element to 2, we would have to update the highlighted nodes in the following diagram. This process is more intuitive than querying.
...
\Function{Update}{$i$, $z$, range $[a,b]$}
	...
	\If{$a = b$}
		...
		\State \Return
	\EndIf
	\State \Call{Update}{$i$, $y$, $\left[a,\floor{\frac{a+b-1}{2}}\right]$}
	\State \Call{Update}{$i$, $y$, $\left[\floor{\frac{a+b-1}{2}}+1,b \right]$}
\EndFunction
\end{algorithmic}
\end{minipage}
...
Now all we need is a way for us to easily store the sum values associated with each node. We can clearly use a structure with two child pointers, but we can also exploit the fact that the segment tree structure is a balanced binary search tree. Therefore, we can store the tree like we store a heap. The root is labeled 1, and for each node with label $index$, the left child is labeled with $index \cdot 2$ and the right child with $index \cdot 2 + 1$. Here's some sample code. Note that in actual implementation, it is easier to zero-index the original array and denote ranges by an open upper bound: $[a, b)$.
