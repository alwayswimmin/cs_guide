...
Java implements a min heap with the \texttt{PriorityQueue}. This class, like \texttt{LinkedList}, also implements \texttt{Queue}. C++ implements a max heap with the \texttt{priority\_queue}. The functions for heaps in both languages are nearly identical to those for queues.
...
\begin{center}
	...
	\textit{push}		&	\texttt{boolean offer(E e)}	&	\texttt{void push (const value\_type\& val)} \\
    \textit{pop}			&	\texttt{E poll()}		&	\texttt{void pop()}	\\
    \textit{front}		&	\texttt{E peek()}		&	\texttt{value\_type\& top()}	\\
    \end{tabular}
\end{center}
